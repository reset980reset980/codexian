...
- **Codexian Galaxy 3D**: 볼트의 Markdown 노트, 내부 링크, 태그, 폴더를 3D 은하형 지식 그래프로 탐색합니다.

## Codexian Galaxy 3D

`Codexian Galaxy 3D 열기` 명령이나 orbit 리본 아이콘으로 별도 그래프 뷰를 열 수 있습니다.

- 노트는 별/입자로 표시되고, 폴더 또는 루트 주제별로 은하 군집을 만듭니다.
- 내부 링크는 금색 연결선, 태그 공유 관계는 청록색 수집 빛으로 표시합니다.
- 검색, 주제 필터, 노드 수 제한, 재초점, 자동회전, 수집 빛 ON/OFF, 2D/3D 전환을 제공합니다.
- 노드를 클릭하면 제목, 폴더, 링크/백링크, 태그, 파일 경로가 오른쪽 패널에 표시됩니다.
- `노트 열기`와 `컨텍스트 고정`으로 그래프 탐색을 Codexian 채팅/숙고 컨텍스트와 연결합니다.
